-- Migration 007: Chat Tables (Override Phase 3 definitions)
-- Creates tables for real-time chat functionality in P2P transactions
-- This migration overrides the Phase 3 chat tables with the correct structure

-- Drop existing chat tables if they exist (from Phase 3)
DROP TABLE IF EXISTS chat_messages CASCADE;
DROP TABLE IF EXISTS chat_rooms CASCADE;

-- Chat rooms table (correct structure for our implementation)
...
-- Chat messages table (correct structure for our implementation)
...
CREATE INDEX idx_chat_rooms_transaction_id ON chat_rooms(transaction_id);
CREATE INDEX idx_chat_rooms_dispute_id ON chat_rooms(dispute_id);
...
-- (Note: Using a conditional approach since orders table might not exist yet)
DO $$
BEGIN
